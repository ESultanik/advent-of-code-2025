...
""", result=21)
@challenge(day=7)
def times_split(lines: list[str]) -> int:
    beams: list[bool] = [c.upper() == 'S' for c in lines[0]]
    splits = 0
    for line in lines[1:]:
        new_beams: list[bool] = list(beams)
...
def propagate(lines: list[str]) -> Iterator[list[str]]:
    prev_line: list[str] = ['.'] * len(lines[0])
    for line in lines:
        new_line: list[str] = []
        for col, c in enumerate(line):
            if c == '.' and prev_line[col] == '|' or (col > 0 and prev_line[col-1] == '^') or (
                    col < len(prev_line) - 1 and prev_line[col+1] == '^') or prev_line[col] == 'S':
                new_line.append('|')
            else:
                new_line.append(c)
        yield new_line
...
@challenge.next(result=40)
def times_split(lines: list[str]) -> int:
    lines = list(propagate(lines))
    paths: list[int] = [
        [0, 1][c == '|']
        for c in lines[-1]
    ]
    for line in lines[-2:0:-1]:
        ...
        for col, c in enumerate(line):
            if c == '.':
                ...
                new_paths.append(paths[col])
            elif c == '^':
                new_paths.append(paths[col-1] + paths[col+1])
        paths = new_paths
    return sum(
        paths[col]
